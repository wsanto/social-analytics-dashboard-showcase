...
const complexEmotions = ["wonder", "curiosity", "anticipation", "trust"]
...
const generateTimeSeriesData = (timeRange: string) => {
  ...
  return Array.from({ length: points }, (_, i) => {
    ...
    return {
      ...
      value: Math.random() * 35 + 25 + Math.sin(i / 6) * 12,
    }
  })
}
...
export function ComplexEmotionsChart() {
  const { topics } = useTopics()
  const [selectedTopic, setSelectedTopic] = useState("")
  const [selectedEmotion, setSelectedEmotion] = useState("wonder")
    ...
    <div className="space-y-4">
      <div>
        <div className="flex items-center gap-2">
          <h3 className="text-lg font-semibold">Complex Emotions Over Time</h3>
          <InfoTooltip metricKey="complexEmotionalStates" />
        </div>
        <p className="text-sm text-muted-foreground">Track wonder, curiosity, anticipation, and trust trends</p>
      </div>
      ...
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span>
          {selectedEmotion.charAt(0).toUpperCase() + selectedEmotion.slice(1)} for {selectedTopic}
        </span>
      </div>
    </div>
  )
